...
(function () {
    // Prevent duplicate initializations or running on ai-tutor page
    if (window.__ZOOMDZ_AI_ASSISTANT_INITIALIZED__) return;
    if (window.location.pathname.includes('ai-tutor') || window.location.href.includes('ai-tutor')) return;
...
[[NAV:transactions:فتح المحفظة والمعاملات]]`;
    ...
    async function sendUserMessage(text) {
        ...
        if (!token || token === 'guest_token') {
            ...
            appendMessage(`🔒 **تنبيه:** استخدام خدمات الذكاء الاصطناعي متاح فقط للمسجلين بالمنصة.\n\nيرجى [إنشاء حساب جديد أو تسجيل الدخول](/?login=true) للتمكن من التحدث مع مساعد Zoomy AI! ✨`, 'bot');
            return;
        }
        ...
        try {
            ...
            if (res.status === 401 || res.status === 403) {
                appendMessage(`🔒 **تنبيه:** يرجى [تسجيل الدخول أو إنشاء حساب جديد](/?login=true) لاستخدام الذكاء الاصطناعي.`, 'bot');
                return;
            }
