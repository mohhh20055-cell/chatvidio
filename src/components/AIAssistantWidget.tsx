...
export default function AIAssistantWidget() {
  ...
  const sendMessage = async () => {
    ...
    if (!token || token === 'guest_token') {
      alert('🔒 عذراً، لا يمكنك استخدام الذكاء الاصطناعي إلا بعد تسجيل الدخول أو إنشاء حساب.');
    ...
    try {
      ...
      if (!response.ok) {
        setMessages([...newMessages, { role: 'assistant', content: data.error || 'يرجى تسجيل الدخول لاستخدام الذكاء الاصطناعي.' }]);
      } else {
