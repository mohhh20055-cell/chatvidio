...
/**
 * ✅ إرسال إشعار بالبريد الإلكتروني للأستاذ عند وصول موعد البث للالتحاق وبدء البث
 */
async function sendStreamStartReminderEmail(toEmail, toName, subjectName, offerId) {
    try {
        const sanitizedEmail = sanitizeInput(toEmail);
        const sanitizedName = sanitizeInput(toName);
        const sanitizedSubject = sanitizeInput(subjectName);
        const streamUrl = `${platformUrl}/teacher-dashboard.html`;

        console.log(`📧 [Stream Reminder] Sending start reminder email for "${sanitizedSubject}" to teacher:`, sanitizedEmail);

        const emailResult = await sendEmail({
            to: sanitizedEmail,
            subject: `🔴 حان موعد بث درس "${sanitizedSubject}" - ZoomDz`,
            html: `
                <!DOCTYPE html>
                <html dir="rtl" lang="ar">
                <head>
                    ...
                    <style>
                        ...
                        .header h1 { color: #0f5cbf; font-size: 1.8rem; margin: 8px 0; }
                        .content { color: #1a2332; line-height: 1.8; font-size: 1rem; }
                        .btn { display: inline-block; background: #ef4444; color: white; padding: 14px 40px; border-radius: 50px; text-decoration: none; font-weight: 700; margin: 20px 0; }
                        .btn:hover { background: #dc2626; }
                        .footer { text-align: center; color: #94a3b8; font-size: 0.8rem; margin-top: 30px; border-top: 1px solid #edf2f7; padding-top: 20px; }
                        .emoji { font-size: 3rem; margin-bottom: 10px; }
                        .alert-box { background: #fef2f2; border-radius: 12px; padding: 20px; border-right: 4px solid #ef4444; margin: 15px 0; }
                        .alert-box p { color: #991b1b; margin: 0; font-weight: 700; }
                    </style>
                </head>
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <h2>مرحباً أستاذ ${sanitizedName} 👋</h2>
                            <div class="alert-box">
                                <p style="font-size: 1.1rem;">⏰ حان الآن موعد بدء بث درس "${sanitizedSubject}"!</p>
                            </div>
                            <p>نود إعلامك بأن موعد الحصة المباشرة قد حان. يرجى الالتحاق بالمنصة الآن وبدء البث المباشر لكي يتمكن الطلاب من الانضمام إليك.</p>
                            <div style="text-align: center;">
                                <a href="${streamUrl}" class="btn">🚀 الالتحاق وبدء البث الآن</a>
                            </div>
                            <p style="font-size: 0.85rem; color: #64748b;">أو تفضل بزيارة المنصة: <a href="${platformUrl}">${platformUrl}</a></p>
                        </div>
                        <div class="footer">
                            <p>© 2026 ZoomDz - منصة التعليم الجزائرية</p>
                            <p style="font-size: 0.75rem;">هذا بريد آلي لتنبيهك بموعد الحصة، يرجى عدم الرد عليه</p>
                        </div>
                    </div>
                </body>
                </html>
        ...
        if (!emailResult.success) {
            logger.error('❌ Error sending stream start reminder email:', emailResult.error);
            return false;
        }

        console.log('✅ Stream start reminder email sent successfully to:', sanitizedEmail);
        return true;
    } catch (error) {
        logger.error('❌ Error sending stream start reminder email:', error.message);
        return false;
    }
}
