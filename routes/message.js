...
// ============================================================
// رفع مرفق رسالة (صور، مستندات، PDF)
// ============================================================
router.post('/upload', authenticate, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, error: 'يرجى تحديد ملف أو صورة للرفع' });
        }
        const uploadResult = await uploadToSupabase(req.file, 'messages');
        if (!uploadResult || !uploadResult.url) {
            return res.status(500).json({ success: false, error: 'فشل حفظ الملف على السيرفر' });
        }
        ...
        logger.error('خطأ في رفع مرفق الرسالة:', e.message);
        return res.status(500).json({ success: false, error: e.message || 'خطأ في الرفع' });
    }
});
...
router.post('/send', authenticate, [
    ...
    try {
        ...
        const { sender_id, sender_type, receiver_id, receiver_type, message, reply_to_id, reply_to_text, reply_to_sender, file_url, file_name, file_size, file_type } = req.body;
        ...
        try {
            newMessage = await insert('messages', {
                ...msgPayload,
                ...(file_url ? { file_url, file_name, file_size, file_type } : {}),
        ...
        if (newMessage && file_url) {
            memoryAttachments.set(newMessage.id, { file_url, file_name, file_size, file_type });
    ...
    try {
        ...
        if (messagesList.length > 0) {
            ...
            for (const msg of messagesList) {
                ...
                // إرفاق المرفقات والصور من الذاكرة كاحتياط
                if (memoryAttachments.has(msg.id)) {
                    const att = memoryAttachments.get(msg.id);
                    msg.file_url = msg.file_url || att.file_url;
                    msg.file_name = msg.file_name || att.file_name;
