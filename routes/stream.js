...
router.post('/start-jitsi-stream', authenticate, authorize(['teacher']), [
    ...
    try {
        ...
        // ✅ حفظ في قاعدة البيانات (جدول jitsi_rooms)
        await supabase
            .from('jitsi_rooms')
            .insert({
                offer_id: offer_id,
...
router.post('/save-link', authenticate, authorize(['teacher']), [
    ...
    body('platform').isIn(['google-meet', 'microsoft-teams', 'jitsi', 'other']).withMessage('منصة غير صالحة')
], async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
...
router.post('/add-all-students/:offer_id', authenticate, authorize(['teacher']), [
    ...
    try {
        ...
        res.json({ 
            ...
            message: 'تم إضافة ' + addedCount + ' طالب إلى البث مع كلمات مرور فريدة'
        });
    } catch (error) {
...
router.get('/teacher-stream/:offer_id/:teacher_id', async (req, res) => {
    try {
        ...
        // ✅ عرض صفحة البث مع iframe (للمعلم)
        res.send(`
            <!DOCTYPE html>
            <html dir="rtl" lang="ar">
            <head>
