...
router.post('/create', authenticate, [
    ...
    try {
        ...
        logger.error('خطأ في إنشاء رمز الإحالة:', error.message);
...
router.get('/info', async (req, res) => {
    try {
        ...
        const { data: studentReferrer } = await supabase
            .from('students')
            .select('id, referral_code, full_name, profile_image, profile_url')
        ...
        return res.json({
            ...
            profile_image: referrer.profile_url || referrer.profile_image || null,
            referral_link: `${PLATFORM_DOMAIN}?ref=${referrer.referral_code}`
        });
    } catch (error) {
        logger.error('خطأ في جلب معلومات الإحالة:', error.message);
        res.status(500).json({ success: false, error: 'حدث خطأ في الخادم' });
    }
});
...
router.get('/info/:user_id/:role', authenticate, [
    ...
    try {
        ...
        if (req.user.userId !== parseInt(user_id) || req.user.role !== role) {
            return res.status(403).json({ success: false, error: 'غير مصرح لك بدرس معلومات الإحالة' });
        }
...
router.post('/open-gift-box', authenticate, authorize(['student']), [
    ...
    try {
        ...
        logger.error('خطأ في فتح صندوق الهدايا:', error.message);
        res.status(500).json({ success: false, error: 'حدث خطأ في الخادم' });
    }
});
...
router.get('/gift-box-status/:student_id', authenticate, authorize(['student']), [
    ...
    try {
        ...
        if (req.user.userId !== parseInt(student_id) || req.user.role !== 'student') {
            return res.status(403).json({ success: false, error: 'غير مصرح لك بدرس حالة صندوق الهدايا' });
        }
        ...
        logger.error('خطأ في جلب حالة صناديق الهدايا:', error.message);
        res.status(500).json({ success: false, error: 'حدث خطأ في الخادم' });
    }
});
...
router.get('/teacher-stats/:teacher_id', authenticate, authorize(['teacher']), [
    ...
    try {
        ...
        if (req.user.userId !== parseInt(teacher_id) || req.user.role !== 'teacher') {
            return res.status(403).json({ success: false, error: 'غير مصرح لك بدرس إحصائيات الإحالة' });
        }
        ...
        logger.error('خطأ في جلب إحصائيات الإحالة:', error.message);
        res.status(500).json({ success: false, error: 'حدث خطأ في الخادم' });
    }
});
