...
const { formatMeetOrZoomUrl } = require('../utils/googleMeet');
...
router.post('/buy-tokens', authenticate, authorize(['student', 'teacher']), async (req, res) => {
    try {
        ...
        const currentTokens = user.ai_tokens !== undefined && user.ai_tokens !== null ? user.ai_tokens : 5; // Default to 5 free tokens
...
router.get('/balance/:student_id', authenticate, authorize(['student']), [
    ...
    try {
        ...
        res.json({
            ...
            ai_tokens: student.ai_tokens !== undefined && student.ai_tokens !== null ? student.ai_tokens : 5,
