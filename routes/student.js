// ============================================================
// مسارات الطالب - Student Routes (معدل بالكامل - متوافق مع نظام المستوى التعليمي)
// ============================================================
...
// ============================================================
// تحديث الملف الشخصي للطالب (بدون تعديل المستوى التعليمي)
// ============================================================
router.post('/update-profile-with-social', authenticate, authorize(['student']), upload.fields([
    ...
    try {
        ...
        const { 
            ...
            phone,
            full_name
            // ❌ تم إزالة education_level - لا يمكن تعديله من هنا
        } = req.body;
...
router.post('/create-session', authenticate, authorize(['student']), [
    ...
    try {
        ...
        // التحقق من أن العرض غير محذوف أو منتهي
        if (offer.status === 'cancelled') {
            return res.status(400).json({ success: false, error: 'هذا العرض ملغى' });
        }

        // التحقق من عدم وجود حجز سابق لنفس العرض
        ...
        if (existingSession?.data) {
            if (existingSession.data.payment_status === 'paid') {
...
// ============================================================
// تأكيد الدفع (للحجوزات المجانية أو بعد الدفع)
// ============================================================
router.post('/confirm-payment', authenticate, authorize(['student']), [
    ...
    try {
        ...
        // جلب العرض للتأكد من أنه مجاني أو مدفوع
        ...
        // إذا كان العرض مدفوعاً، قم بخصم المبلغ من محفظة الطالب
        if (!offer.is_free && offer.price > 0) {
            ...
            // إضافة المبلغ إلى حساب الأستاذ
            const teacher = await getOne('teachers', 'id', offer.teacher_id);
            if (teacher) {
                await update('teachers', offer.teacher_id, {
                    balance: (teacher.balance || 0) + offer.price,
                    total_earned: (teacher.total_earned || 0) + offer.price
                });
            }
        }
...
router.post('/cancel-session/:session_id', authenticate, authorize(['student']), [
    ...
    try {
        ...
        if (session.payment_status === 'paid') {
            return res.status(400).json({ success: false, error: 'لا يمكن إلغاء حجز مدفوع' });
        }

        // حذف الجلسة
        await remove('sessions', 'id', session_id);
